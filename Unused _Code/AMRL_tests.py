...
while keep_going:
    ...
    ENV_SmallLake_det.reset()

    agent_var2 = AMRL_Variant_2(ENV_SmallLake_det, nmbr_particles = 100)

    (r_avg, results) = agent_var2.run(2000, True)   
    print(r_avg)
    if r_avg < 200:
        print ("failed to find optimal strategy")

        vis = visualiser(ENV_SmallLake_det, agent_var2)
