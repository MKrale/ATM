...
eps=1000
runs=1
for alpha_plan in 0.5
do
    for p_real in $(seq 0.05 0.05 1)
    do
        p_alpha=0.05
        alpha_real=$(echo "scale=5; $p_alpha/$p_real" | bc)
        python3 ./Run.py -alg ATM                   -alpha_real $alpha_real -alpha_plan $alpha_plan                     -env uMV -env_var $p_alpha -nmbr_eps $eps -nmbr_runs $runs &
        wait
        python3 ./Run.py -alg ATM_RMDP              -alpha_real $alpha_real -alpha_plan $alpha_plan                     -env uMV -env_var $p_alpha -nmbr_eps $eps -nmbr_runs $runs &
        python3 ./Run.py -alg ATM_Robust            -alpha_real $alpha_real -alpha_plan $alpha_plan                     -env uMV -env_var $p_alpha -nmbr_eps $eps -nmbr_runs $runs &
        python3 ./Run.py -alg ATM_Control_Robust    -alpha_real $alpha_real -alpha_plan $alpha_plan -alpha_measure 0    -env uMV -env_var $p_alpha -nmbr_eps $eps -nmbr_runs $runs &    
        python3 ./Run.py -alg ATM_Control_Robust    -alpha_real $alpha_real -alpha_plan $alpha_plan -alpha_measure 1    -env uMV -env_var $p_alpha -nmbr_eps $eps -nmbr_runs $runs &
        python3 ./Run.py -alg ATM_Control_Robust    -alpha_real $alpha_real -alpha_plan $alpha_plan -alpha_measure -1   -env uMV -env_var $p_alpha -nmbr_eps $eps -nmbr_runs $runs 
        # Note: alpha_measure = alpha_plan!
    done
done
